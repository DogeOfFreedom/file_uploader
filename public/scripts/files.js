...
const uploadFileSubmit = async (folderId) => {
  ...
  if (!file.value) {
    ...
    const { exists } = await fetch(
      `http://dogeoffreedom-file-uploader.adaptable.app/file/exists?filename=${filename}&folderId=${folderId}&type=${type}`,
      {
        method: "post",
      }
    )
...
const createFolderSubmit = async (folderId) => {
  ...
  if (value.length >= 1) {
    ...
    const { exists } = await fetch(
      `http://dogeoffreedom-file-uploader.adaptable.app/folder/exists?foldername=${value}&folderId=${folderId}`,
      {
        method: "post",
      }
    )
